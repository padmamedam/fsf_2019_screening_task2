...
#        QIcon.setThemeName('gnome')
        ...
        self.windowList = []
        self.recentFileActs = []
        self.settings = QSettings('Axel Schneider', 'Fossee')
...
#        self.tableView.setSelectionMode(QAbstractItemView.MultiSelection)
    ...
    def createMenuBar(self):
        ...
        self.filemenu.addAction(QIcon.fromTheme("document-open"), "Load",  self.loadCsv ) 
        self.filemenu.addAction(QIcon.fromTheme("document-save"), "Save",  self.saveOnQuit) 
        
        self.filemenu.addSeparator()
        self.filemenu.addAction(QIcon.fromTheme("add"), "add data",  self.addRow)
        ...
        #self.editmenu.addAction(QIcon.fromTheme("add"), "add data",  self.addRow)
        
        self.editmenu.addAction(QIcon.fromTheme("Scatter"),"select columns",self.copyByContext)

        
        self.editmenu.addAction(QIcon.fromTheme("edit-edit"), "Edit",  self.replaceThis)
        #self.editmenu.addAction(findThisAction)

        self.editmenu1=bar.addMenu("Scatter plot")
        #s=App()
        #m = PlotCanvas()
        
        
        self.editmenu1.addAction(QIcon.fromTheme("Scatter"),"Scatter plot", self.plot)
        
        self.editmenu2=bar.addMenu("line plot")
        self.editmenu2.addAction(QIcon.fromTheme("Scatter"),"line plot", self.plot1)

        self.editmenu3=bar.addMenu("smooth lines")
       
        self.editmenu3.addAction(QIcon.fromTheme("Scatter"),"smooth line", self.plot2) 
...
#            print(mystring)
    ...
    def addRow(self):
        ...
        self.tableView.setRowCount(count + 1)
        self.tableView.resizeRowsToContents()
        self.isChanged = True
        if self.tableView.rowCount() == 0:
            self.addColumn()
    ...
    def contextMenuEvent(self, event):
        ...
        if self.tableView.selectionModel().hasSelection():
            ...
            replaceThisAction.triggered.connect(lambda: self.replaceThis(event))
            # find all
            ...
            self.menu.addSeparator()
            self.menu.addAction(QIcon.fromTheme("edit-edit"), "Edit",  self.replaceThisAction)
    ...
    def plot(self):
        ...
        train_data = pd.read_csv(z)
        
        
        plt.title('scatter plot')
        plt.xlabel('x-axis')
        plt.ylabel('y-axis')
        plt.scatter(train_data[p[0][0]],train_data[p[0][1]])
        plt.show()
    ...
    def plot1(self):
        ...
        p.append(self.a)
        train_data = pd.read_csv(z)
        train_inputs = train_data['Manufacturer']
        fig=plt.figure()
        #data = [random.random() for i in range(25)]
        #ax = self.figure.add_subplot(121)
        #ax.plot(train_inputs, 'r-')
        plt.title(' line plot')
        plt.xlabel('x-axis')
        plt.ylabel('y-axis')
        plt.plot(train_data[p[0][0]],train_data[p[0][1]])
